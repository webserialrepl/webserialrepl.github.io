...
`,timeout:s=3e3}={}){var u;const c=await navigator.serial.requestPort();await c.open({baudRate:t}),this.serialPort=c,await new Promise(g=>setTimeout(g,200));const m=(u=c.writable)==null?void 0:u.getWriter();if(await(m==null?void 0:m.write(new TextEncoder().encode(i))),m==null||m.releaseLock(),!c.readable)throw new Error("Port is not readable");const w=c.readable.getReader(),r=(async()=>{let g="";for(;;){const{value:l,done:f}=await w.read();if(f)break;if(g+=new TextDecoder().decode(l),g.includes(">>>"))return g}return g})(),n=await Promise.race([r,new Promise((g,l)=>setTimeout(()=>l(new Error("Timeout")),s))]);return w.releaseLock(),console.log("\u63A5\u7D9AOK",n),c}async openPortWithRetry222({usbVendorId:t=null,baudRate:i=115200,maxRetries:s=1,testCommand:c=`ping
`,testTimeoutMs:m=4e3}={}){let w=0,r;for(;w<s;){w++,console.log(`\u30DD\u30FC\u30C8\u30AA\u30FC\u30D7\u30F3\u8A66\u884C ${w}/${s}...`);try{if(r=await navigator.serial.requestPort({filters:t?[{usbVendorId:t}]:[]}),await r.open({baudRate:i}),await new Promise(l=>setTimeout(l,400)),r.writable){const l=r.writable.getWriter(),f=new TextEncoder;await l.write(f.encode(c)),await l.close().catch(()=>{}),l.releaseLock()}if(!r.readable)throw new Error("Port is not readable");const n=r.readable.getReader(),u=setTimeout(()=>{console.warn("\u30C6\u30B9\u30C8\u53D7\u4FE1\u30BF\u30A4\u30E0\u30A2\u30A6\u30C8:",m,"ms"),n.cancel().catch(()=>{})},m);let g="";for(;;){const{value:l,done:f}=await n.read();if(console.log("Received chunk:",l,f),f||(g+=new TextDecoder().decode(l),g.includes(">>>")))break}if(clearTimeout(u),n.releaseLock(),g.length>0)return console.log("\u30DD\u30FC\u30C8\u63A5\u7D9A\uFF06\u5FDC\u7B54\u78BA\u8A8DOK",g),this.serialPort=r,r;console.warn("\u5FDC\u7B54\u306A\u3057\u3001\u30DD\u30FC\u30C8\u3092\u9589\u3058\u307E\u3059"),await r.close()}catch(n){if(console.error("\u8A66\u884C\u30A8\u30E9\u30FC:",n),r)try{await r.close()}catch{}}await new Promise(n=>setTimeout(n,1e3))}throw new Error("\u63A5\u7D9A\u5931\u6557: \u898F\u5B9A\u56DE\u6570\u30EA\u30C8\u30E9\u30A4\u3057\u307E\u3057\u305F\u304C\u5FDC\u7B54\u304C\u3042\u308A\u307E\u305B\u3093")}async reopen(){this.serialPort&&(await this.serialPort.close(),await this.serialPort.open({baudRate:115200}),console.warn("\u30DD\u30FC\u30C8\u3092\u518D\u5EA6\u958B\u304D\u76F4\u3057\u307E\u3059\u3002"))}async openPort(){try{const t=await this.openPortWithRetry({baudRate:115200,testCommand:`ping
...
`,ri=(e="value")=>(t,i)=>{const s=t.constructor,c=s.prototype.attributeChangedCallback;s.prototype.attributeChangedCallback=function(m,w,r){var n;const u=s.getPropertyOptions(e),g=typeof u.attribute=="string"?u.attribute:e;if(m===g){const l=u.converter||ei,b=(typeof l=="function"?l:(n=l==null?void 0:l.fromAttribute)!=null?n:ei.fromAttribute)(r,u.type);this[e]!==b&&(this[i]=b)}c.call(this,m,w,r)}},Vt=Q`
